...
### Fixed
- No `maxlength` on any form field, and no server-side length check, even though most fields map to fixed-width `VARCHAR` columns. Consequences of an over-long value: under strict SQL mode the main `INSERT` failed and the applicant saw only the generic "Something went wrong while saving your application" (after the uploaded files were rolled back); under non-strict mode the value was silently truncated; and for academic records — whose `$wpdb->insert()` results are never checked — the row was silently lost or truncated while the rest of the application was reported as successful.
- Added `adm_mgr_get_field_limits()` and `adm_mgr_get_academic_field_limits()` as the single source of truth for column widths (Full Name/Father's/Mother's Name/Last School/Course 255; Email 100; contact numbers 20; Pin Code 10; Blood Group 5; Nationality 50; Country/State of Domicile 100; academic Exam Name/Board 255, Year 10, Class/Division 100, % Marks 20). Used by both `adm_mgr_maxlength_attr()` (emits the HTML `maxlength` on each input, including newly added academic rows since they clone the first row) and the new `adm_mgr_check_field_lengths()`, so the browser limit and the server limit cannot drift apart. The Aadhar input also gets `maxlength="12"` to match its existing exact-12-digit rule.
- `adm_mgr_check_field_lengths()` runs in `adm_mgr_handle_submission()` right after the email validation and before any file is written (no orphaned uploads on rejection). It measures values exactly as they will be stored: after the same `sanitize_text_field()` / `sanitize_email()` processing, and for the block-letters name after `adm_mgr_to_uppercase()` (uppercasing can lengthen text, e.g. "ß" → "SS"), counting characters rather than bytes (`mb_strlen`) since VARCHAR widths are in characters under utf8mb4. Academic rows are only checked when they would actually be saved (non-empty exam name), mirroring the insert loop. Returns a specific message for the first offending field, e.g. "WhatsApp No. is too long (maximum 20 characters)." or "Academic record — Year is too long (maximum 10 characters).".
- Deliberately not limited: TEXT columns (addresses, subjects), `sex`/`category` (already whitelisted), and Aadhar (already exact-12-digit validated). The Email limit (100) is a database-width limit, narrower than the 254 characters `is_email()` accepts; widening the column would be a schema change and was not part of this fix.
- Verified against a real MariaDB 10.11 server in STRICT mode using the CREATE TABLE statements parsed straight out of the plugin source (not retyped): every mapped limit equals the actual column width (22 of 22); for every field, a value of exactly N characters passes the check and inserts, while N+1 is rejected by the check and independently rejected by MySQL with "Data too long" (22 of 22), using multi-byte characters to confirm character-not-byte counting. Edge cases verified: 127 × "ß" (254 chars uppercased) accepted vs 128 × "ß" (256) rejected — and the DB rejects that uppercased value too; empty-exam-name academic rows skipped; second academic row checked; crafted non-string/array values don't fatal; surrounding whitespace not counted. Ran the real v1.1.17 and v1.1.18 handlers side by side against WordPress core's actual `sanitize_text_field()`/`sanitize_email()`/`is_email()` source: over-limit inputs that v1.1.17 let through now get the specific message, at-limit and typical values behave as before, and the v1.1.17 invalid-email message still takes precedence. Rendered the real form and parsed the HTML: all 22 fields carry the correct `maxlength`, and the only text/tel/email input without one is the honeypot.
- No database schema changes; `ADM_MGR_DB_VERSION` unchanged. If a column width in `adm_mgr_activate()` is ever changed, the two limit maps must be updated to match.
